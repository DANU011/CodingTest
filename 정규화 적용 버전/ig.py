...
# ───────────────────────────────────────────────────────
# 2. 데이터 및 모델 로드
shap_data = np.load("shap_input/shap_input_201807_201812_20250606_071900.npz", allow_pickle=True)
...
model.load_state_dict(torch.load("best_churn_model_pth/best_churn_model_201807_201812_20250606_071900.pth", map_location=device))
model.train()
...
def run_ig_and_plot(index, label):
    ...
    plt.tight_layout()
    plt.savefig(f"ig_meta_top10_{label}.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"ig_seq_top10_{label}.png", dpi=300, bbox_inches='tight')
    plt.close()
